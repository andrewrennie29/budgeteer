class ExpenseCategoriesController < ApplicationController
  ...
  def index
    @expense_categories = ExpenseCategory.order(category: :asc)
  end

  # GET /expense_categories/1
  # GET /expense_categories/1.json
  def show
    @expenses = @expense_category.expenses.in_period(Budget.find_by_name('General').start_date..Budget.find_by_name('General').end_date).order(expense_date: :asc)
  end
  ...
  private
    ...
    def expense_category_params
      params.require(:expense_category).permit(:category, :icon)
    end
end
